...
function render({ filename, files, metalsmith, settings }) {
  const overridePath = Object.keys(settings.overrideExt)
    .reduce((longestMatch, override) =>
      match(filename, override).length > 0 && override.length > longestMatch.length
        ? override
        : longestMatch
    , '')
  const [base, ...extensions] = overridePath
    ? ((fn, op, ovrExt) => {
          const dir = fn.substring(0, fn.lastIndexOf('/')+1);
          const [base, ...oldExts] = fn.split('/').slice(-1)[0].split('.');
          const exts = ovrExt[op].split('.').filter(s => s.trim().length > 0);
          const oldExt = oldExts.join('.');

          debug(`user wants to override .${oldExt} extension with ${ovrExt[op]} extension for ${filename}`);
          return [dir + base, ...exts];
      })(filename, overridePath, settings.overrideExt)
    : filename.split('.');
  ...
  // Stop if we don't want to transform this extension or further
  if (settings.stopAt === ext) {
    debug(`user wants to stop at ${ext} extension for ${filename}`);
    return Promise.resolve();
  }
...
function validate({ filename, files, overrideExt = {} }) {
  ...
  const overridePath = Object.keys(overrideExt)
    .reduce((longestMatch, override) =>
      match(filename, override).length > 0 && override.length > longestMatch.length
        ? override
        : longestMatch
  ...
  const extension = (overrideExt[overridePath] || filename).split('.').pop();
...
module.exports = options => (files, metalsmith, done) => {
  ...
  // do not allow empty path in overrideExt
  delete settings.overrideExt[''];
  ...
  const validFiles = matchedFiles.filter(filename => validate({ filename, files, overrideExt: settings.overrideExt }));
